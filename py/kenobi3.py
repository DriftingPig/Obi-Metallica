...
class SimDecals(LegacySurveyData):
    ...
    def __init__(self, dataset=None, survey_dir=None, metacat=None, simcat=None,
        ...
        self.stamp_stat_fn = kwargs['stamp_stat_fn']
        self.brickname = kwargs['brickname']
        self.sim_noise_level = kwargs['sim_noise_level']
...
class SimImage(DecamImage):
    ...
    def __init__(self, survey, t):
        ...
        self.t = t
        self.survey = survey
        self.stamp_stat_fn = survey.stamp_stat_fn
        self.brickname = survey.brickname
        self.sim_noise_level = survey.sim_noise_level
        ...
        dirs={'proj':'/global/project/projectdirs/cosmo/staging/decam',
                  'proja':'/global/projecta/projectdirs/cosmo/staging/decam'}
        #t21 = time_builtin.clock()
        #sys.stdout.flush()
        if not "DECam_CP" in self.imgfn:
            self.imgfn = self.imgfn.replace("decam/","decam/DECam_CP/")
        if not os.path.exists(self.imgfn):
            ...
            found = glob('%s/DECam_CP/*/%s' % (dirs['proja'],base), recursive=False)
            if len(found) == 0:
                 found = glob('%s/DECam_CP/*/%s' % (dirs['proj'],base), recursive=False)
            if len(found) == 0:
    ...
    def make_poission_noise_image(self,tim):
        #image with just poission noise
        mean, median, std = sigma_clipped_stats(tim.data, sigma=6.0)
        tim.data = np.random.randn(tim.data.shape[0],tim.data.shape[1])*std
        return tim
    def make_sim_image(self,tim):
        #TODO
        #print(self.brickname)
        #shot the image to 0
        #import pdb;pdb.set_trace()
        mean, median, std = sigma_clipped_stats(tim.data, sigma=3.0)
        noise = np.random.randn(tim.data.shape[0],tim.data.shape[1])*std*2.2
        tim.data = np.zeros(tim.data.shape)
        brickname = self.brickname
        #name_for_run = 'sim_fig_test'
        #rs_type = 'rs_0'
        tractor_fn = '/global/project/projectdirs/cosmo/data/legacysurvey/dr3/tractor/'+brickname[:3]+'/tractor-'+brickname+'.fits'
        print(tractor_fn)
        #import pdb;pdb.set_trace()
        tractor = fits.getdata(tractor_fn)
        objstamp = BuildStamp(tim, seed=self.survey.seed,
                              camera=self.t.camera,
                              gain=self.t.gain,exptime=self.t.exptime)
        cat = fits_table()
        cat.set('ra',tractor['ra'])
        cat.set('dec',tractor['dec'])
        cat.set('gflux',tractor['DECAM_FLUX'][:,1])
        cat.set('rflux',tractor['DECAM_FLUX'][:,2])
        cat.set('zflux',tractor['DECAM_FLUX'][:,4])
        cat.set('id',np.arange(len(tractor)))
        e1 = []
        e2 = []
        n = []
        rhalf = []
        for i in range(len(tractor)):
            real_source_type = tractor['type'][i]
            if real_source_type in ['SIMP', 'PSF']:
                n.append(1)
                rhalf.append(0.5)
                e1.append(tractor['shapeExp_e1'][i])
                e2.append(tractor['shapeExp_e2'][i])
            elif real_source_type in ['COMP','EXP']:
                n.append(1)
                rhalf.append(tractor['shapeExp_r'][i])
                e1.append(tractor['shapeExp_e1'][i])
                e2.append(tractor['shapeExp_e1'][i])
            elif real_source_type == 'DEV':
                n.append(4)
                rhalf.append(tractor['shapeDev_r'][i])
                e1.append(tractor['shapeDev_e1'][i])
                e2.append(tractor['shapeDev_e2'][i])
            else:
                raise
        ...
        objstamp = BuildStamp(tim, seed=self.survey.seed,
                              camera=self.t.camera,
                              gain=self.t.gain,exptime=self.t.exptime)
        tim_image = galsim.Image(np.zeros(tim.data.shape))
        tim_invvar = galsim.Image(tim.getInvvar())
        tim_dq = galsim.Image(tim.dq)
        for i in range(len(cat)):
            obj = cat[i]
            print('[IMG]Drawing source %d, n=%.2f, rhalf=%.2f, e1=%.2f, e2=%.2f'%(i,cat.n[i],cat.rhalf[i],cat.e1[i],cat.e2[i]))
            stamp = objstamp.elg(obj)
            print('[IMG] Finished Drawing : id=%d band=%s dbflux=%f addedflux=%f' %
                (obj.id,objstamp.band,
                 obj.get(objstamp.band+'flux'),stamp.array.sum()))
    ...
    def use_dr3_model_image(self,tim):
        #import pdb;pdb.set_trace()
        from astrometry.util.resample import resample_with_wcs
        mean, median, std = sigma_clipped_stats(tim.data, sigma=6.0)
        #import pdb;pdb.set_trace()
        noise = np.random.randn(tim.data.shape[0],tim.data.shape[1])*std+self.sim_noise_level*0.00001
        print("NOISE_LEVEL--%f"%(self.sim_noise_level*0.00001))
        #from legacypipe.runs import get_survey
        #parser= get_parser()
        #arg_result = parser.parse_args(args=None)
        #survey = get_survey('decam',
        #                    survey_dir=arg_result.survey_dir,
        #                    output_dir=arg_result.outdir,
        #                    cache_dir=arg_result.outdir)

        brick = self.survey.get_brick_by_name(self.brickname)
        targetwcs = wcs_for_brick(brick, W=3600, H=3600, pixscale=0.262)
        
        (Yo,Xo,Yi,Xi,img) = resample_with_wcs(targetwcs, tim.subwcs, [tim.data])
        #this does not work. It gives me a weird image, not sure what is run though. 
        tim.data = np.zeros(tim.data.shape)
        band = tim.band.strip()
        brickname = self.brickname
        coadd_fn = '/global/project/projectdirs/cosmo/data/legacysurvey/dr3/coadd/'+brickname[:3]+'/'+brickname+'/'+'legacysurvey-'+brickname+'-model-'+band+'.fits.gz'
        coadd = fits.getdata(coadd_fn)
        tim.data = np.zeros(tim.data.shape)
        tim.data[Yi,Xi] = coadd[Yo,Xo]
        tim.data += noise
        #import pdb;pdb.set_trace()
        #tim_image = galsim.Image(tim.getImage())
        #print(tim_image.bounds)
        #coadd_image = galsim.Image(coadd)
        #overlap = tim_image.bounds
        #tim.data += coadd_image[overlap].array
        return tim
    ...
    def get_tractor_image(self, **kwargs):
        ...
        if tim is None: # this can be None when the edge of a CCD overlaps
            return tim
        tim = self.use_dr3_model_image(tim)
        #debug_hui
        #shot the image to 0
        #tim.data = np.zeros(tim.data.shape)
...
class BuildStamp():
      ...
      def elg(self,obj):
          ...
          #if mod0.sum()<0.001 and mod0.sum()>-0.001:
          #      import pdb;pdb.set_trace()
...
def get_parser():
    ...
    parser.add_argument('--sim_noise_level', type=float, default=0,help='noise level on the simulated image')
    return parser
...
def do_one_chunk(d=None):
    ...
    kw.update(stamp_stat_fn=d['stamp_stat_fn'])
    kw.update(brickname = d['brickname'])
    kw.update(sim_noise_level = d['sim_noise_level'])
...
def main(args=None):
    ...
    if (os.path.exists(tractor_fn) &
       ...
       return 0 #sys.exit(0)
    stamp_stat_fn = os.path.join(args.outdir,
                       'tractor',args.brick[:3],args.brick,
                       rsdir,
                       'stamp-stat-%s.txt' %args.brick)
    #import pdb;pdb.set_trace()
    #print(stamp_stat_fn)
    ...
    kwargs=dict(Samp=Samp,\
                ...
                args=args,\
                sim_noise_level = args.sim_noise_level,\
                stamp_stat_fn=stamp_stat_fn)
